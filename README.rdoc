...
=== Typical Controller Class
    class Home < Fedora
      namespace '/' # Tells Fedora to set these actions to the root of the URL
...
If your controller doesn't contain "views_from" Fedora will simply look in :views/<controller name>/ ("namespace" acts in the same exact way). Be sure to check out the app.rb and config.ru source.
